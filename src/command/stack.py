...
class DupCommand(Command):
    """
    Command class for DUP operation (duplicates the top item on the stack).
    """
...
class DropCommand(Command):
    """
    Command class for DROP operation (removes the top item from the stack).
    """
    def execute(self, stack):
        if len(stack) < 1:
...
class SwapCommand(Command):
    """
    Command class for SWAP operation (swaps the top two items).
    """
    def execute(self, stack):
        if len(stack) < 2:
...
class OverCommand(Command):
    """
    Command class for OVER operation (copies the second item to the top).
    """
    def execute(self, stack):
        if len(stack) < 2:
